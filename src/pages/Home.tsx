...
const Home: React.FC = () => {
  ...
  const services = [
    { title: 'Rent Property', desc: 'Premium flats & shops. Verified for safety.', icon: <HomeIcon className="text-accent" size={24} />, link: '/properties?type=Rent', color: 'from-blue-500/20 to-cyan-500/20', span: 'col-span-1 md:col-span-2' },
    { title: 'Buy/Sell', desc: 'Transparent real estate deals in Patna.', icon: <Key className="text-accent" size={24} />, link: '/properties?type=Buy', color: 'from-purple-500/20 to-pink-500/20', span: 'col-span-1' },
    { title: 'Consultation', desc: 'Expert advice from Sanjeev Pandey.', icon: <Zap className="text-accent" size={24} />, link: '/about', color: 'from-green-500/20 to-emerald-500/20', span: 'col-span-1 md:col-span-1' }
  ];
  ...
  return (
    <div className="overflow-hidden bg-white dark:bg-gray-900" ref={containerRef}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            ...
            <motion.div style={{ opacity }} className="lg:col-span-6 text-center lg:text-left flex flex-col items-center lg:items-start">
              ...
              <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-primary dark:text-white leading-[1.1] mb-5 tracking-tight uppercase italic">
                Fast. Trusted. <br />
                <span className="text-accent text-glow">Expert Service.</span>
              </h1>
              
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-base md:text-lg text-gray-600 dark:text-gray-400 mb-8 max-w-lg font-hindi leading-relaxed">
                पटना में घर ढूँढना अब और भी आसान। हम लेकर आए हैं आपके लिए <span className="text-primary dark:text-white font-black border-b-2 border-accent/30 pb-0.5">72 घंटों का भरोसा</span>।
              </motion.p>
              
              <motion.div initial={{ y: 15, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.7 }} className="flex flex-wrap justify-center lg:justify-start gap-4">
                <a href="tel:9934072003" className="btn-shiny !px-6 !py-3 text-sm shadow-xl">Call Now</a>
                <a href="https://wa.me/919334966607" className="glass-morph !bg-white/10 text-primary dark:text-white font-black px-6 py-3 rounded-xl border-white/20 text-sm flex items-center gap-2 hover:bg-accent/10 transition-all"><MessageCircle size={18} className="text-green-500" /> WhatsApp</a>
              </motion.div>
            ...
            {/* Slider Side - Compacted height */}
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }} className="lg:col-span-6 w-full lg:max-w-xl mx-auto">
              <HeroSlider properties={sliderProperties} />
            </motion.div>
          </div>
        </div>
      </section>

      {/* Featured Collections - Reduced Padding */}
      <section className="py-20 dark:bg-[#020D1A]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div {...fadeInUpProps} className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            ...
            <Link to="/properties" className="btn-shiny !bg-accent !text-primary !py-2.5 !px-6 shadow-accent/10 text-[9px] uppercase tracking-widest">View All Listings</Link>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <motion.div {...fadeInUpProps} className="flex flex-col md:flex-row justify-between items-center mb-16 gap-8">
            <div className="text-center md:text-left">
              ...
              <h3 className="text-3xl md:text-5xl font-black tracking-tight uppercase italic">Why Families Trust Us.</h3>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {reviews.map((r, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="bg-white/5 backdrop-blur-xl p-7 rounded-[2rem] border border-white/10 relative group">
      ...
      <section className="py-24 relative overflow-hidden bg-white dark:bg-gray-900">
        ...
        <div className="max-w-5xl mx-auto px-4 text-center text-white relative z-10"><motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}><p className="text-accent font-black uppercase tracking-[0.5em] mb-6 text-[8px]">Ready to move?</p><h2 className="text-4xl md:text-6xl font-black mb-10 tracking-tight leading-none italic uppercase">Stop Searching. <br /> <span className="text-accent underline decoration-white/20 underline-offset-4">Start Living.</span></h2><div className="flex flex-col items-center gap-8"><div className="flex flex-wrap justify-center gap-6"><motion.a whileHover={{ scale: 1.05 }} href="tel:9934072003" className="bg-accent text-primary font-black py-4 px-10 rounded-[2rem] text-lg shadow-2xl hover:bg-white transition-all uppercase tracking-widest">9934-0720-03</motion.a><motion.a whileHover={{ scale: 1.05 }} href="https://wa.me/919334966607" className="bg-white/10 backdrop-blur-xl border-2 border-white/20 text-white font-black py-4 px-10 rounded-2xl text-lg hover:bg-accent hover:text-primary transition-all uppercase tracking-widest">WhatsApp Chat</motion.a></div><p className="text-[9px] text-white/50 font-black uppercase tracking-[0.2em] font-hindi">पूरी जानकारी और बेहतरीन डील्स के लिए अभी संपर्क करें।</p></div></motion.div></div>
      </section>
